# El modelo puede definirse directamente desde este script principal, incluyendo sus hiperparámetros personalizados.
# En este caso, se utiliza MLPClassifier con un mayor número de iteraciones para asegurar la convergencia del entrenamiento.
# Esta estrategia permite al usuario ajustar el comportamiento del modelo sin necesidad de modificar la clase SHAPExplainer.
...
INSTANCE_INDEX = 10
SHAP_FILE = "shap_values_iris_mlp.pickle"
NEW_INSTANCE = [5.0, 2.0, 5.1, 1.8]
mlp_model = lambda: MLPClassifier(max_iter=1000, random_state=42)

df = pd.read_csv(CSV_PATH)
CLASS_NAMES = {i: name for i, name in enumerate(["setosa", "versicolor", "virginica"])}

if __name__ == "__main__":
    explainer = SHAPExplainer(CSV_PATH, mlp_model, TARGET_COLUMN)
